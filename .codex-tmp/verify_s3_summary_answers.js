...
const generatorPath = 'data/practice-generators/s3.js';
...
if (!keys.length) throw new Error('no s3 configs registered');

for (const key of keys) {
  if (process.env.VERBOSE_S3_VERIFY === '1') {
  ...
  if (config.__generatorFingerprint !== 's3-bundle-v20260706-summary-v1') {
...
console.log(`verified summaryAnswers for ${keys.length} s3 configs`);
