...
const generatorPath = 'data/practice-generators/s2.js';
...
if (!keys.length) throw new Error('no s2 configs registered');
if (context.window.__s2BundleFingerprint !== 's2-bundle-v20260706-summary-v1') {
  throw new Error(`wrong s2 fingerprint: ${context.window.__s2BundleFingerprint}`);
}

for (const key of keys) {
  if (process.env.VERBOSE_S2_VERIFY === '1') {
...
console.log(`verified summaryAnswers for ${keys.length} s2 configs`);
