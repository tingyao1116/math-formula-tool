...
SOURCE_DOCX = Path(r"E:\備份資料\張快數學\哈特例高中數學講義\108課綱\教材重點\1-8轉.docx")
SOURCE_REF = "1-8轉.docx"
CHAPTER_CODE = "s1-2-3"
ID_PREFIX = "q-s1-2-3-line-circle-relation-word01"
...
WORK_DIR = ROOT / "exports" / "s1-2-3-line-circle-relation-word"
PLAIN_PATH = WORK_DIR / "1-8轉.plain.txt"
...
TOPIC_TITLES = {
    "s1-2-3-line-circle-relation-core": "直線與圓的位置關係",
    "senior-circle-line-relation": "圓與直線的關係",
    "senior-circle-line-three-cases": "相交、相切、不相交判別",
    "senior-circle-tangent-formulas": "切線公式整理",
    "senior-circle-tangent-from-external-point": "圓外一點作切線與切線長",
    "senior-circle-line-construction-by-slope": "已知斜率作圓的切線",
    "senior-circle-family-and-radical-axis-advanced": "圓系方程式與根軸進階",
    "senior-circle-line-distance-discriminant-s123": "圓與直線關係的雙判法",
    "senior-circle-tangent-at-point-formula-s123": "圓上一點切線方程",
    "senior-circle-line-relation-workflow-s123": "圓與直線三種關係判別流程",
    "senior-circle-tangent-construction-two-methods-s123": "作圓切線兩種主流程（已知斜率／已知外點）",
    "senior-circle-chord-of-contact-equation-s123": "切點弦方程與切線束",
    "senior-circle-pencil-through-intersections-s123": "經兩圓交點的圓系建立法",
    "senior-circle-chord-tangent-power-s123": "冪定理（割線、切線）",
}
...
def classify_topic(theme: str, question: str, explanation: str) -> str:
    ...
    if theme == "pencil":
        if any(word in blob for word in ["根軸", "二圓", "C1", "C₁", "AB為直徑", "連心線"]):
            return "senior-circle-family-and-radical-axis-advanced"
        return "senior-circle-pencil-through-intersections-s123"

    if any(word in blob for word in ["切點弦", "極線", "切線束", "AB方程式", "A,B兩點分別做圓C的切線", "切點為A與B"]):
        return "senior-circle-chord-of-contact-equation-s123"

    if any(word in blob for word in ["PA×PB", "PA}×", "割線", "冪", "切線段長", "作的切線段長"]):
        return "senior-circle-chord-tangent-power-s123"

    if theme == "tangent":
        if any(word in blob for word in ["圓上一點", "切點", "以A為切點", "切於A", "切於點", "證明"]):
            return "senior-circle-tangent-at-point-formula-s123"
        if any(word in blob for word in ["圓外一點", "過P", "兩切線", "作兩切線", "自點P", "外部一點"]):
            return "senior-circle-tangent-from-external-point"
        if any(word in blob for word in ["平行", "垂直", "斜率", "x軸", "y軸", "相切之直線", "相切的直線"]):
            return "senior-circle-line-construction-by-slope"
        return "senior-circle-tangent-formulas"

    if theme == "point":
        if any(word in blob for word in ["切線段長", "切點分別", "兩切線", "作二切線", "作圓C的兩切線", "圓外一點"]):
            return "senior-circle-tangent-from-external-point"
        if any(word in blob for word in ["冪", "PA×PB", "割線"]):
            return "senior-circle-chord-tangent-power-s123"
        return "s1-2-3-line-circle-relation-core"

    if any(word in blob for word in ["相交於兩點", "交於兩點", "相切", "相離", "不相交", "無交點", "有相異兩交點", "相割"]):
        if any(word in blob for word in ["範圍", "何時", "判別", "D:", "D：", "D>0", "D<0", "D=0"]):
            return "senior-circle-line-relation-workflow-s123"
        return "senior-circle-line-distance-discriminant-s123"

    if any(word in blob for word in ["距離最大", "距離最小", "最大值", "最小值", "直線距離"]):
        return "senior-circle-line-distance-discriminant-s123"

    if any(word in blob for word in ["半圓", "sqrt{1-x", "sqrt{4-x", "上半圓"]):
        return "senior-circle-line-three-cases"

    return "senior-circle-line-relation"


def difficulty_for(role: str, question: str) -> str:
    text = compact_for_match(question)
    if any(word in text for word in ["證明", "範圍", "最大", "最小", "切線", "圓系", "根軸", "外接圓", "投射", "相交於兩點", "相切", "相離"]):
        return "進階"
    if role == "隨堂練習":
        return "中等"
    return "基礎"
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Convert 1-8 line-circle-relation DOCX into question/link JSONL import files.")
