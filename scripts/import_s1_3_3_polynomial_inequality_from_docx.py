...
SOURCE_DOCX = Path(r"E:\備份資料\張快數學\哈特例高中數學講義\108課綱\教材重點\1-11轉.docx")
SOURCE_REF = "1-11轉.docx"
CHAPTER_CODE = "s1-3-3"
ID_PREFIX = "q-s1-3-3-polynomial-inequality-word01"
...
WORK_DIR = ROOT / "exports" / "s1-3-3-polynomial-inequality-word"
PLAIN_PATH = WORK_DIR / "1-11轉.plain.txt"
...
TOPIC_TITLES = {
    "s1-3-3-polynomial-inequality-core": "多項式不等式解法（符號表）",
    "senior-polynomial-inequality-main": "多項式不等式",
    "senior-polynomial-inequality-linear-quadratic": "一次與二次不等式",
    "senior-polynomial-inequality-quadratic-sign-conditions": "二次函數恆正恆負條件",
    "senior-polynomial-inequality-higher-order": "高次不等式判號法",
    "senior-polynomial-rational-radical-inequality": "分式與根式不等式補充",
    "senior-polynomial-inequality-sign-chart-s133": "數線判號法",
    "senior-polynomial-inequality-parameter-cases-s133": "參數不等式分類討論",
    "senior-polynomial-inequality-boundary-inclusion-s133": "不等式邊界點納入判斷",
    "senior-polynomial-inequality-sign-chart-workflow-s133": "多項式不等式判號表流程",
    "senior-polynomial-inequality-root-multiplicity-s133": "重根奇偶與過零行為",
    "senior-quadratic-global-sign-criteria-s133": "二次式全域正負條件總表",
    "senior-rational-inequality-excluded-values-s133": "分式不等式與禁值處理",
}
...
def frac_denominators(text: str) -> List[str]:
    denominators: List[str] = []
    pos = 0
    while True:
        start = text.find("\\frac", pos)
        if start == -1:
            return denominators
        pos = start + len("\\frac")
        while pos < len(text) and text[pos].isspace():
            pos += 1
        _, pos = _read_braced(text, pos)
        while pos < len(text) and text[pos].isspace():
            pos += 1
        denominator, pos = _read_braced(text, pos)
        if denominator:
            denominators.append(denominator)


def has_variable_denominator(text: str) -> bool:
    return any(re.search(r"[A-Za-z]", denominator) for denominator in frac_denominators(text))
...
def classify_topic(theme: str, question: str, explanation: str) -> str:
    ...
    if "分式不等式" in qblob or has_variable_denominator(question):
        return "senior-rational-inequality-excluded-values-s133"

    if any(word in qblob for word in ["參數", "實數a", "實數k", "實數m", "a∈R", "m為一實數", "a之範圍", "k的範圍", "m的範圍"]):
        if any(word in qblob for word in ["恆成立", "所有實數", "沒有實數解", "無實解", "對任意實數", "對所有實數", "圖形恆"]):
            return "senior-quadratic-global-sign-criteria-s133"
        return "senior-polynomial-inequality-parameter-cases-s133"

    if theme == "quadratic" and any(word in qblob for word in ["恆成立", "所有實數", "沒有實數解", "無實解", "對任意實數", "對所有實數", "圖形恆"]):
        return "senior-quadratic-global-sign-criteria-s133"

    if any(word in qblob for word in ["sqrt", "根式", "絕對值", "|x", "距離"]):
        return "senior-polynomial-rational-radical-inequality"

    if theme == "quadratic" and any(word in qblob for word in ["同號", "判別式", "b²-4ac", "b2-4ac"]):
        return "senior-quadratic-global-sign-criteria-s133"

    if any(word in qblob for word in ["≥", "<=", "≤", ">=", "不大於", "至少", "介於", "範圍", "等於", "邊界", "整數解", "以上"]):
        if theme == "higher":
            return "senior-polynomial-inequality-boundary-inclusion-s133"
        if theme == "quadratic":
            return "senior-polynomial-inequality-linear-quadratic"

    if theme == "linear":
        return "senior-polynomial-inequality-linear-quadratic"

    if theme == "quadratic":
        if any(word in qblob for word in ["b2-4ac", "b²-4ac", "二次函數", "D=", "判別式"]):
            return "senior-polynomial-inequality-quadratic-sign-conditions"
        return "senior-polynomial-inequality-linear-quadratic"

    if any(word in qblob for word in ["偶次", "奇次", "¹⁹", "⁹⁰", "重根", "平方", "不等式(x+3)"]):
        return "senior-polynomial-inequality-root-multiplicity-s133"

    if any(word in blob for word in ["數線", "判號", "正負", "因式分解"]) or (
        theme == "higher" and any(word in qblob for word in ["解下列不等式", "解不等式", "不等式("])
    ):
        return "senior-polynomial-inequality-sign-chart-s133"

    if theme == "higher":
        return "senior-polynomial-inequality-higher-order"

    return "s1-3-3-polynomial-inequality-core"


def difficulty_for(role: str, question: str) -> str:
    text = compact_for_match(question)
    if any(word in text for word in ["參數", "範圍", "恆成立", "所有實數", "分式", "根式", "絕對值", "應用", "至少", "不大於", "最大", "最小"]):
        return "進階"
    if role == "隨堂練習":
        return "中等"
    return "基礎"
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Convert 1-11 polynomial-inequality DOCX into question/link JSONL import files.")
