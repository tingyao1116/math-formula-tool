...
SOURCE_DOCX = Path(r"E:\備份資料\張快數學\哈特例高中數學講義\108課綱\教材重點\1-9轉.docx")
SOURCE_REF = "1-9轉.docx"
CHAPTER_CODE = "s1-3-1"
ID_PREFIX = "q-s1-3-1-polynomial-function-word01"
TOPIC_ID = "s1-3-1-polynomial-function-core"
TOPIC_TITLE = "多項式函數與餘式定理"
...
WORK_DIR = ROOT / "exports" / "s1-3-1-polynomial-function-word"
PLAIN_PATH = WORK_DIR / "1-9轉.plain.txt"
...
def is_topic_heading(line: str) -> bool:
    return bool(re.match(r"^主題\d+[：:]?.+", line))
...
def difficulty_for(role: str, question: str) -> str:
    ...
    if any(word in text for word in ["證明", "餘式", "因式", "整除", "最高次", "最低次", "插值", "牛頓", "拉格朗日", "參數", "所有", "範圍"]):
        return "進階"
    if role == "隨堂練習":
        return "中等"
    return "基礎"
...
def build_preview(questions: List[Dict], links: List[Dict], applied: Dict | None = None) -> Dict:
    ...
    return {
        ...
        "targets": [
            {"topic_id": topic_id, "topic_title": TOPIC_TITLE if topic_id == TOPIC_ID else topic_id, "count": count}
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Convert 1-9 polynomial-function DOCX into question/link JSONL import files.")
