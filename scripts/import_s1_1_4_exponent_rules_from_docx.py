...
SOURCE_DOCX = Path(r"E:\備份資料\張快數學\哈特例高中數學講義\108課綱\教材重點\1-4轉.docx")
SOURCE_REF = "1-4轉.docx"
CHAPTER_CODE = "s1-1-4"
ID_PREFIX = "q-s1-1-4-exponent-rules-word01"

TARGET_TOPIC_ID = "s1-1-4-exponent-rules"
TARGET_TOPIC_TITLE = "指數律與分數指數"
...
WORK_DIR = ROOT / "exports" / "s1-4-exponent-rules-word"
PLAIN_PATH = WORK_DIR / "1-4轉.plain.txt"
...
def difficulty_for(role: str, question: str) -> str:
    ...
    if any(word in text for word in ["最大", "最小", "函數", "細菌", "草履蟲", "布袋蓮", "人口", "半衰期", "颱風", "音階"]):
        return "進階"
    if role == "隨堂練習":
        return "中等"
    return "基礎"
            ...
            rows.append(
                {
                    ...
                    "tags": [
                        CHAPTER_CODE,
                        "指數",
                        f"topic:{TARGET_TOPIC_ID}",
                        f"role:{role_slug}",
                        f"source-section:{current_topic}",
                        f"module:{TARGET_TOPIC_TITLE}",
                        "word_import",
                    ],
                }
            )
...
def make_links(questions: List[Dict]) -> List[Dict]:
    ...
    return [
        {
            "id": link_id_for(question["id"], question["target_id"]),
            "title": f"{question['id']} -> {question['target_id']}",
...
def build_preview(questions: List[Dict], links: List[Dict], applied: Dict | None = None) -> Dict:
    ...
    return {
        ...
        "targets": [
            {"topic_id": topic_id, "topic_title": TARGET_TOPIC_TITLE, "count": count}
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Convert 1-4 exponent-rules DOCX into question/link JSONL import files.")
