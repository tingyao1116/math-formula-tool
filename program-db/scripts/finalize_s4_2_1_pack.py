...
SOURCE_PACK = "_inspect-4A-5"
CHAPTER_CODE = "s4-2-1"
CHAPTER_TITLE = "平面方程式"
SOURCE_DOC_NAME = "4A-5轉.docx"
EXTRACTED_MD_NAME = "4A-5轉.md"
...
RAW_INSPECT_MEDIA_RE = re.compile(
    r"\(?\.?[\\/]+program-db[\\/]+imports[\\/]+packs[\\/]+_inspect-4A-5[\\/]+assets[\\/]+media[\\/][^)\]\s]+(?:\.(?:png|jpg|jpeg|emf|wmf))\)?",
    re.I,
)
EDITORIAL_RE = re.compile(r"【龍騰自命題】|【觀念補充】|【進階補充】")
MATRIX_ENV_RE = re.compile(r"\\begin\{([a-z]*matrix|array)\}(.*?)\\end\{\1\}", re.S)
SECTION_FALLBACK = "主題1：平面方程式"
...
def infer_missing_question_text(marker: str, section: str, explanation_text: str) -> str:
    if section.startswith("主題1") and marker.startswith("範例4"):
        return "如圖，依光學反射關係求鏡面平面 E 的方程式。"
    if section.startswith("主題3") and marker.startswith("隨堂練習"):
        return "依解析與附圖，求題目所問的平面或最值結果。"
    if explanation_text:
...
def infer_formula_id(section: str, question_text: str, explanation_text: str) -> str:
    ...
    if section.startswith("主題1"):
        if any(token in blob for token in ("交線", "平面束", "E_{1}", "E_1", "E_{2}", "E_2", "λ", "lambda")):
            return "senior-plane-pencil-equation-s421"
        if any(token in blob for token in ("截距", "\\frac{x}", "\\frac{y}", "\\frac{z}", "x截距", "y截距", "z截距")):
            return "senior-plane-intercept-form-s421"
        if any(token in blob for token in ("A", "B", "C", "三點", "行列式", "AB", "AC")) and any(
            token in blob for token in ("平面方程式", "過點", "共平面")
        ):
            return "senior-plane-equation-three-point-form-s421"
        return "senior-plane-equation-main-s421"

    if section.startswith("主題2"):
        return "senior-plane-distance-parallel-s421"

    if any(token in blob for token in ("交角平分面", "平行平面", "距離", "投影點", "對稱點", "最小值", "中垂平面")):
        return "senior-plane-distance-parallel-s421"
    return "senior-plane-distance-parallel-s421"
...
def make_review_needed(records: list[dict]) -> str:
    review_rows = []
    for row in records:
        if "【待複核】" in row["question_text"] or row["question_text"].startswith("如圖"):
    ...
    lines = [
        ...
        "",
        f"- 題數：{len(records)}",
        "- 重點清理：平面方程式、兩平面的夾角、點到平面的距離與投影。",
        "- 圖片：已補 `wmf/emf -> png` sidecar。",
        "",
        "## 建議人工複核",
    ]
    if review_rows:
        lines.extend(review_rows[:10])
    else:
