...
PLAIN_SUP_RE = re.compile(r"\^(?!\{)([^\\^{}\n$]{1,24})\^")
PLAIN_SUB_RE = re.compile(r"~(?!\{)([^~{}\n$]{1,24})~")
...
def convert_caret_subscript_notation(text: str) -> str:
    source = str(text or "")
    source = PLAIN_SUP_RE.sub(lambda m: f"^{{{m.group(1).strip()}}}", source)
    source = PLAIN_SUB_RE.sub(lambda m: f"_{{{m.group(1).strip()}}}", source)
    return source
...
def cleanup_fragment(text: str) -> str:
    ...
    source = UNDERLINE_BLANK_RE.sub("____________", source)
    ...
    source = convert_caret_subscript_notation(source)
    source = source.replace("  ", " ")
...
def main():
    parser = argparse.ArgumentParser(description="Build s1-1-4 question pack from extracted markdown.")
    parser.add_argument("--markdown", required=True)
    parser.add_argument("--chapter-code", default="s1-1-4")
    parser.add_argument("--source-ref", default="source/1-4轉.docx")
    ...
    manifest = {
        "chapter_code": args.chapter_code,
        "chapter_title": "指數",
        "source_files": [{"path": args.source_ref, "role": "primary_docx", "note": "原始 Word 題源"}],
        "extracted_files": [
