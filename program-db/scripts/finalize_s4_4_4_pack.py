...
SOURCE_PACK = "_inspect-4A-10"
CHAPTER_CODE = "s4-4-4"
CHAPTER_TITLE = "平面線性變換與二階方陣"
SOURCE_DOC = "4A-10轉.docx"
SOURCE_MD = "4A-10轉.md"
FORMULA_ID = "s4-4-4-linear-transform-2x2-core"
...
RAW_MEDIA_RE = re.compile(
    r"\(?\.?[\\/]+program-db[\\/]+imports[\\/]+packs[\\/]+_inspect-4A-10[\\/]+assets[\\/]+media[\\/][^)\]\s]+(?:\.(?:png|jpg|jpeg|emf|wmf))\)?",
    re.I,
)
...
def build_review(records: list[dict]) -> str:
    lines = [
        ...
        "",
        f"- 目前共 {len(records)} 題，全部先掛在網站既有核心 `{FORMULA_ID}`。",
        "- 這章網站目前沒有更細的 `s4-4-4` 分支，所以先依 `主題1~4` 保留來源段落，不硬拆自建分支。",
        "- `wmf/emf` 已補成 `.png` sidecar，圖像題可直接在前端檢查。",
        "- 建議優先檢查含大量圖形變換示意的題目，例如旋轉、鏡射、推移後的直線與曲線題。",
        "",
        "## Source sections",
        "",
    ]
    ...
    lines.extend(
        [
            ...
            "## Manual review",
            "",
            "- `q-s4-4-4-0028`：原始 Word 題幹幾乎完全依附圖面，這版已補成保底題幹，建議前端再核對。",
            "- `q-s4-4-4-0029`：題幹文字很短，主要依附前圖判讀，建議和上一題一起看。",
        ]
    )
    lines.append("")
    return "\n".join(lines)
...
def main():
    markdown = read_text(PACKS_DIR / SOURCE_PACK / "extracted" / SOURCE_MD)
    ...
    (target_root / "review-needed.md").write_text(build_review(records), encoding="utf-8")
