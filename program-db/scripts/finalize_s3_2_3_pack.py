...
SOURCE_PACK = "_inspect-3A-3"
SOURCE_DOC = "3A-3轉.docx"
SOURCE_MD = "3A-3轉.md"
CHAPTER_CODE = "s3-2-3"
CHAPTER_TITLE = "對數函數"
...
SOURCE_LINE_RE = re.compile(
    r"(?m)^【[^】]*(?:自命題|段考|期中|期末|模擬|講義|女中|高中|一中|附中|學測|北一女|龍騰|康熹|南一|翰林|嘉義高中)[^】]*】\s*$"
)
...
def formula_id_for(section: str, title: str, question_text: str, explanation_text: str) -> str:
    ...
    if has_any(text, ["芮氏規模", "地震", "噪音", "分貝", "面積等級", "世界各國", "聲音的強度", "W/m^2", "震央", "瓦斯汽笛"]):
        return "senior-logarithmic-function-applications-s323"

    if section.startswith("主題4："):
        if has_any(text, ["對稱於直線y = x", "反函數", "f^{-1}", "反函數記做", "反函數為", "互為反函數"]):
            if has_any(text, ["定義域", "值域", "x > 4", "|x| > 1", "x > 1", "圖形與其反函數"]):
                return "senior-logarithmic-function-inverse-domain-s323"
            return "senior-logarithmic-function-reflection-map-s323"
        return "senior-logarithmic-function-main-s323"

    if section.startswith("主題3："):
        if has_any(text, ["交點", "圖形", "作", "圖解", "|log", "2^x", "y = ", "有幾個實根", "實數解個數"]):
            return "senior-logarithmic-function-inequality-graph-s323"
        return "senior-logarithmic-equations-inequalities-s323"

    if section.startswith("主題2："):
        if has_any(text, ["交點", "圖形", "有多少實根", "實根個數", "作", "|log", "解的個數"]):
            return "senior-logarithmic-function-intersection-root-count-s323"
        return "senior-logarithmic-equations-inequalities-s323"

    if section.startswith("主題1："):
        if has_any(text, ["最小值", "最大值", "算幾", "凹口向下", "凹口向上", "平均不等式"]):
            return "senior-logarithmic-function-concavity-mean-inequality-s323"
        if has_any(text, ["交點", "有幾個實根", "實根個數", "f(x) = g(x)", "解的個數"]):
            return "senior-logarithmic-function-intersection-root-count-s323"
        if has_any(text, ["|log", "|x|", "平移", "對稱x軸", "對稱y軸", "沿x軸方向移動", "沿y軸方向移動", "作圖", "描繪", "圖形同", "對稱於x軸", "對稱於y軸"]):
            if has_any(text, ["f( - x)", "對稱原點", "f(x - h)", "f( - x)", "複合"]):
                return "senior-logarithmic-function-compound-transform-s323"
            return "senior-logarithmic-function-transformations-s323"
        if has_any(text, ["大小", "比較", "b > a > 1", "何者為真", "何者正確", "log_a x", "log_b x", "遞增", "遞減"]) and not has_any(
            text, ["方程式", "不等式", "解", "最小值", "最大值"]
        ):
            return "senior-logarithmic-function-comparison-s323"
        if has_any(text, ["定義域", "值域", "漸近線", "恆在y軸右側", "x恆大於0", "過點(1，0)", "有意義"]):
            return "senior-logarithmic-function-domain-range-s323"
        if has_any(text, ["互為反函數", "對稱於直線x - y = 0", "對稱於直線y = x", "y = a^x", "y = log_a x"]):
            return "senior-logarithmic-function-reflection-map-s323"
        if has_any(text, ["圖形", "漸近線", "通過", "凹口", "遞增函數", "遞減函數"]):
            return "senior-logarithmic-function-graph-properties-s323"
        return "s3-2-3-log-function-graph-core"
...
def build_review(records: list[dict]) -> str:
    ...
    lines.extend(
        [
            ...
            "## Notes",
            "",
            "- 這章改用章節專用 parser，直接依 `主題1/2/3/4` 與 `範例/隨堂練習` 重建切題。",
...
def build_pack(base_dir: Path):
    ...
    for index, block in enumerate(blocks, start=1):
        ...
        formula_id = formula_id_for(block["section"], title, question_text, explanation_text)
