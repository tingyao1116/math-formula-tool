...
SOURCE_PACK = "_inspect-5A-9"
CHAPTER_CODE = "s5-9"
CHAPTER_TITLE = "函數性質的判定"
SOURCE_REF = "source/5A-9轉.docx"

MARKER_RE = re.compile(r"\*\*(隨堂練習|範例\s*\d+|範例\d+|龍騰自命題)\*\*")
UNDERLINE_MARKER_RE = re.compile(r"\[(隨堂練習|龍騰自命題)\]\{\.underline\}")
...
RAW_MEDIA_RE = re.compile(
    r"\.?[\\/]+program-db[\\/]+imports[\\/]+packs[\\/]+_inspect-5A-9[\\/]+assets[\\/]+media[\\/][^)\]\s]+",
    re.I,
)
WRAPPED_MEDIA_BLOCK_RE = re.compile(
    r"!\[(?P<alt>[\s\S]*?)\]\((?P<path>\.?[\\/]+program-db[\\/]+imports[\\/]+packs[\\/]+_inspect-5A-9[\\/]+assets[\\/]+media[\\/][^)]+)\)(?:\{[^{}]*\})?",
    re.S,
)

SPECIAL_INLINE_TEXT = {
    "image1.wmf": "∈",
    "image21.wmf": r"$\lim_{h \to 0^+}$",
    "image22.wmf": r"$\lim_{h \to 0^-}$",
    "image23.wmf": r"$\lim_{h \to 0}$",
    "image30.wmf": r"$\lim_{x \to c}$",
    "image77.wmf": "⇔",
    "image148.wmf": "x = ",
}
SKIP_MEDIA = {"image143.wmf", "image154.emf"}
...
def build_title(marker: str, question_text: str) -> str:
    ...
    if marker == "龍騰自命題" and snippet:
        return clean_question_title(f"龍騰自命題：{snippet}")
    return clean_question_title(snippet or marker)


def difficulty_from_marker(marker: str) -> str:
    return "中" if marker in {"隨堂練習", "龍騰自命題"} else "易"


def category_from_marker(marker: str) -> str:
    return "重要" if marker in {"隨堂練習", "龍騰自命題"} else "基本"
...
def assign_formula_id(section: str, question_text: str, explanation_text: str) -> str:
    ...
    if section.startswith("主題1"):
        branch_tokens = ("遞增區間", "遞減區間", "符號表", "變化表", "f'(x)", "導數值恆為正", "導數值恆為負")
        if any(token in text for token in branch_tokens):
            return "s5-9-monotonicity-branch-sign-chart-method"
        return "s5-9-monotonicity"

    if section.startswith("主題2"):
        branch_tokens = ("最大值", "最小值", "閉區間", "端點", "絕對極大", "絕對極小", "臨界值")
        if any(token in text for token in branch_tokens):
            return "s5-9-extrema-first-derivative-branch-endpoint-check"
        return "s5-9-extrema-first-derivative"

    if section.startswith("主題3"):
        branch_tokens = ("f''", "二階導數", "二階檢定", "凹向", "反曲點", "concave", "inflection")
        if any(token in text for token in branch_tokens):
            return "s5-9-concavity-inflection-branch-second-derivative-test"
        return "s5-9-concavity-inflection"

    if section.startswith("主題4"):
        branch_tokens = ("圖形", "描繪", "作圖如下", "相異實根", "三次函數", "極大值", "極小值", "容積", "最省時")
        if any(token in text for token in branch_tokens):
            return "s5-9-cubic-sketch-branch-sketch-checklist"
        return "s5-9-cubic-sketch"
        ...
        records.append(
            {
                ...
                "source_order": index,
                "tags": [CHAPTER_CODE, f"section:{section}", f"marker:{marker}", "source:5A-9轉.docx"],
            }
        )
    return records
...
def postprocess_records(records: list[dict]) -> list[dict]:
    ...
    row = by_id.get("q-s5-9-0116")
    if row:
        row["question_text"] = row["question_text"].replace(
            "[圖:program-db/assets/question-media/s5-9/image144.png]",
            "[圖:program-db/assets/question-media/s5-9/image142.png]",
        )
        if "[圖:program-db/assets/question-media/s5-9/image144.png]" not in row["explanation_text"]:
...
def copy_support_files(target_root: Path):
    ...
    shutil.copy2(source_root / "source" / "5A-9轉.docx", target_root / "source" / "5A-9轉.docx")
    shutil.copy2(source_root / "extracted" / "5A-9轉.md", target_root / "extracted" / "5A-9轉.md")
...
def build_manifest() -> dict:
    return {
        ...
        "extracted_files": [
            {"path": "extracted/5A-9轉.md", "role": "pandoc_markdown"},
...
def write_review(path: Path, records: list[dict]):
    lines = [
        ...
        "",
        f"- 總題數：{len(records)} 題。",
        "- `image1.wmf`、`image21.wmf`、`image22.wmf`、`image23.wmf`、`image30.wmf`、`image77.wmf`、`image148.wmf` 已人工判定為內聯文字，不再保留成圖片。",
        "- `image6.wmf`、`image8.wmf`、`image9.wmf`、`image10.wmf`、`image11.wmf`、`image34.wmf`、`image36.wmf`、`image37.wmf`、`image38.wmf`、`image40.wmf` 已保留為變化表或圖形圖片。",
        "- 巢狀圖片區塊已拆成正式圖片引用；`image143.wmf` 與 `image154.emf` 為輔助小圖，未列入正式媒體引用。",
        "- 所有正式圖片均統一指向 `program-db/assets/question-media/s5-9/`。",
        "",
    ]
...
def main():
    source_root = PACKS_DIR / SOURCE_PACK
    markdown_text = read_text(source_root / "extracted" / "5A-9轉.md")
