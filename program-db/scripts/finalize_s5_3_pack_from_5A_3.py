...
SOURCE_PACK = "_inspect-5A-3"
CHAPTER_CODE = "s5-3"
CHAPTER_TITLE = "二項分布與幾何分布"
SOURCE_REF = "source/5A-3轉.docx"
...
RAW_MEDIA_RE = re.compile(
    r"\.?[\\/]+program-db[\\/]+imports[\\/]+packs[\\/]+_inspect-5A-3[\\/]+assets[\\/]+media[\\/][^)\]\s]+",
    re.I,
)
...
def replace_inline_images(text: str) -> str:
    ...
    def repl_md(match: re.Match[str]) -> str:
        ...
        if alt in {"≒", "0-01"}:
            return "≈"
        parts: list[str] = []
        if alt and alt not in {"1", "0-01", "≒"} and "龍騰自命題" not in alt:
            parts.append(alt)
        if path:
...
def split_question_and_explanation(raw_text: str) -> tuple[str, str]:
    lines = strip_source_lines(str(raw_text or "").split("\n"))
...
def normalize_text(text: str) -> str:
    ...
    value = value.replace("\\>", ">")
    value = value.replace("。。", "。")
    value = value.replace("0。99", "0.99")
    value = value.replace("0。01", "0.01")
    value = value.replace("0。9", "0.9")
    value = value.replace("0。75", "0.75")
    value = value.replace("0。4", "0.4")
    value = value.replace("0。6", "0.6")
    value = value.replace("0。5", "0.5")
    value = value.replace("0。3010", "0.3010")
    value = value.replace("→", "⇒")
...
def assign_formula_id(section: str, question_text: str, explanation_text: str) -> str:
    ...
    if section.startswith("主題1"):
        if "互斥" in text or "相依" in text:
            return "s5-3-independent-events-branch-independent-vs-mutual"
        return "s5-3-independent-events"
    if section.startswith("主題2"):
        if "伯努利" in text or "標準差" in text or "變異數" in text:
            return "s5-3-binomial-distribution-branch-model-check"
        return "s5-3-binomial-distribution"
    if "無記憶" in text:
        return "s5-3-geometric-distribution-branch-memoryless"
    return "s5-3-geometric-distribution"
        ...
        records.append(
            {
                ...
                "source_order": index,
                "tags": [CHAPTER_CODE, f"section:{section}", f"marker:{marker}", "source:5A-3轉.docx"],
            }
        )
    return records
...
def copy_support_files(target_root: Path):
    ...
    shutil.copy2(source_root / "source" / "5A-3轉.docx", target_root / "source" / "5A-3轉.docx")
    shutil.copy2(source_root / "extracted" / "5A-3轉.md", target_root / "extracted" / "5A-3轉.md")
...
def build_manifest() -> dict:
    return {
        ...
        "extracted_files": [
            {"path": "extracted/5A-3轉.md", "role": "pandoc_markdown"},
...
def write_review(path: Path, records: list[dict]):
    lines = [
        ...
        "- 向量圖 (`.wmf` / `.emf`) 已轉成正式引用的 `.png`，並統一指到 `program-db/assets/question-media/s5-3/`。",
        "- `主題3：幾何分布` 在原始 Word 中只有定義與推廣說明，沒有實際題目，因此本次正式 pack 沒有幾何分布題目。",
        "",
    ]
...
def main():
    source_root = PACKS_DIR / SOURCE_PACK
    markdown_text = read_text(source_root / "extracted" / "5A-3轉.md")
