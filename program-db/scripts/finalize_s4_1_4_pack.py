...
SOURCE_PACK = "_inspect-4A-4"
CHAPTER_CODE = "s4-1-4"
CHAPTER_TITLE = "外積、體積與行列式"
SOURCE_DOC_NAME = "4A-4轉.docx"
EXTRACTED_MD_NAME = "4A-4轉.md"
...
RAW_INSPECT_MEDIA_RE = re.compile(
    r"\(?\.?[\\/]+program-db[\\/]+imports[\\/]+packs[\\/]+_inspect-4A-4[\\/]+assets[\\/]+media[\\/][^)\]\s]+(?:\.(?:png|jpg|jpeg|emf|wmf))\)?",
    re.I,
)
EDITORIAL_RE = re.compile(r"【龍騰自命題】|【觀念補充】|【進階補充】")
MATRIX_ENV_RE = re.compile(r"\\begin\{([vpb]?matrix)\}(.*?)\\end\{\1\}", re.S)
SECTION_FALLBACK = "主題1：外積、面積與體積"
...
def infer_formula_id(section: str, question_text: str, explanation_text: str) -> str:
    ...
    if section.startswith("主題1"):
        if any(token in blob for token in ("右手定則", "垂直", "法向量", "垂線")):
            return "senior-cross-product-right-hand-rule-s414"
        if any(token in blob for token in ("點到直線距離", "直線距離", "距離", "高", "垂足")) and any(
            token in blob for token in ("直線", "AB", "BC", "QR", "投影")
        ):
            return "senior-cross-product-line-distance-s414"
        return "senior-cross-product-volume-main-s414"

    if section.startswith("主題2"):
        return "senior-determinant-3x3-expansion-s414"

    if any(token in blob for token in ("共線", "共平面", "三線共點", "共點", "四點")):
        return "senior-collinear-coplanar-tests-determinant-s414"
    if any(token in blob for token in ("聯立方程", "直線", "面積", "體積", "座標變換", "PQR", "DEF", "ABC")):
        return "senior-collinear-coplanar-tests-determinant-s414"
    return "senior-collinear-coplanar-tests-determinant-s414"
...
def make_review_needed(records: list[dict]) -> str:
    ...
    for row in records:
        ...
        if image_count >= 2 or "共平面" in row["question_text"] or "三線共點" in row["question_text"]:
            review_rows.append(
                f"- `{row['id']}`：建議前端確認圖文搭配與數學排版。"
            )
    ...
    lines = [
        ...
        "",
        f"- 題數：{len(records)}",
        "- 重點清理：外積／面積／體積、三階行列式展開、行列式應用分流。",
        "- 圖片：已補 `wmf/emf -> png` sidecar。",
        "",
        "## 建議人工複核",
    ]
    if review_rows:
        lines.extend(review_rows[:8])
    else:
...
def sync_media_tree(src: Path, dst: Path):
    ...
    for item in sorted(src.iterdir()):
        ...
        try:
            ...
            # Keep an existing locked asset if Windows is holding it open.
