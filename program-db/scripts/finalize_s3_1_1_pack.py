            ...
            blocks.append(
                {
                    ...
                    "section": current_section or "主題1：弧度、弧長與扇形面積",
                    "raw_text": raw_text,
                }
            )
        buffer = []
...
def formula_id_for(section: str, title: str, question_text: str, explanation_text: str) -> str:
    ...
    core_text = "\n".join([title, question_text, explanation_text])
    is_conversion_prompt = has_any(
        core_text,
        ["以度數表示", "以弧度表示", "換算", "度數表之", "弧度表之", "化成弧度量", "化為弧度量"],
    ) or ("°" in core_text and "π" in core_text and not has_any(core_text, ["sin", "cos", "tan", "cot", "sec", "csc"]))

    if is_conversion_prompt and not has_any(
        core_text, ["時針", "分針", "鐘面", "扇形", "周長", "面積", "半徑", "圓錐", "弧長為", "所對之弧長"]
    ):
        return "senior-degree-radian-conversion-s311"

    if has_any(text, ["時針", "分針", "鐘面", "時鐘"]):
        return "senior-clock-hand-angle-model-s311"

    if has_any(text, ["時速", "角速度", "轉速", "每分鐘", "每秒", "公路", "車輪", "皮帶", "滾", "輪心距", "輪"]):
        return "senior-radian-arc-speed-model-s311"

    if has_any(text, ["圓錐", "斜高", "展開", "錐面", "螞蟻", "繞錐", "底半徑"]):
        return "senior-radian-sector-ratio-method-s311"

    if has_any(text, ["同界角", "最大負同界角", "最小正同界角", "第幾象限", "象限角", "正規化"]):
        return "senior-radian-angle-normalization-s311"

    if has_any(text, ["sin", "cos", "tan", "cot", "sec", "csc"]):
        if has_any(text, ["π", "弧度", "同界角", "象限"]):
            return "senior-radian-periodic-conversion-rules-s311"
        return "senior-unit-circle-radian-reference-s311"

    if has_any(text, ["弦", "弓形", "陰影", "著色", "內接", "正方形", "四分之一圓", "弦長", "中央部分", "夾角"]):
        return "senior-radian-chord-arc-angle-relation-s311"

    if has_any(text, ["固定", "定值", "最大", "最小", "證明"]) and has_any(text, ["扇形", "周長", "面積", "半徑", "圓心角"]):
        return "senior-radian-sector-ratio-method-s311"

    if has_any(text, ["扇形", "周長", "面積", "半徑", "圓心角", "弧長", "弧度"]) and has_any(text, ["比例", "圓周", "一周", "展成"]):
        return "senior-radian-sector-ratio-method-s311"

    if has_any(text, ["扇形", "周長", "面積", "半徑", "圓心角", "弧長", "弧度"]):
        return "senior-sector-arc-formulas-s311"

    if has_any(text, ["π", "弧度", "象限", "同界"]):
        return "senior-radian-periodic-conversion-rules-s311"

    return "s3-1-1-radian-arc-core"


def build_preview(records: list[dict]) -> dict:
    by_section: dict[str, list[dict]] = {}
    for record in records:
        section = record.get("source_section") or "主題1：弧度、弧長與扇形面積"
...
def build_review(records: list[dict]) -> str:
    ...
    lines.extend(
        [
            ...
            "## Notes",
            "",
            "- 本章以 `主題1 / 範例 / 隨堂練習` marker 重新切題，不沿用通用 builder 的 24 題結果。",
            "- `範例 -> 基本`、`隨堂練習 -> 重要` 已保留。",
...
def copy_support_files(base_dir: Path, pack_dir: Path) -> tuple[str, str]:
    ...
    source_doc = first_match(source_dir / "source", "3A-4*.docx")
    source_md = first_match(source_dir / "extracted", "3A-4*.md")
...
def build_pack(base_dir: Path):
    ...
    markdown_path = first_match(source_pack / "extracted", "3A-4*.md")
